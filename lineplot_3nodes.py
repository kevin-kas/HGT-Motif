...
sicks=['UC','CD','non']
for sick in sicks:
	for patient_name in os.listdir('3nodes_info_for_plot'):
		if patient_name.split('_')[1]!=sick:
			continue
		table_folder=os.path.join('3nodes_info_for_plot', patient_name)
		for info_table in os.listdir(table_folder):
			with open('3nodes_info_for_plot/'+patient_name+'/'+info_table) as f:
				reader=csv.reader(f)
				dict1=dict()
				for lines in reader:
					if lines==[]:
...
all_time=[]
list_dir=os.listdir('3nodes_info_for_plot')
for i in list_dir:
	path1=os.path.join('3nodes_info_for_plot', i)
...
all_patient=[]
for i in os.listdir('3nodes_info_for_plot'):
